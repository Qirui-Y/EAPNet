...
model_urls = {
    'xception': '/home/jayden/Code/refine_deeplab_xception/model/xception_pytorch_imagenet.pth'#'http://data.lip6.fr/cadene/pretrainedmodels/xception-b5690688.pth'
}
...
class Xception(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.bn2(x)
        x = self.relu(x)
        low_level_feat = x  #[1,64,H/2,W/2]
        #print(low_level_feat.shape)

        x = self.block1(x)
        x = self.relu(x)   #[1,128,H/4,W/4]
        mid_layer = x      #[1,128,H/4,W/4]
        #print(mid_layer.shape)
        x = self.block2(x)   #[1,256,H/8,W/8]
        third_layer = x   #[1,256,H/4,W/4]
        ...
        x = self.block4(x)
        x = self.block5(x)
        x = self.block6(x)  #[1,728,H/16,W/16]
        ...
        x = self.block10(x)
        x = self.block11(x)
        x = self.block12(x)  #[1,728,H/16,W/16]
        ...
        x = self.block16(x)
        x = self.block17(x)
        last_layer = x      #[1,728,H/16,W/16]  
        #print(last_layer.shape)
        x = self.block18(x)
        x = self.block19(x)
        x = self.block20(x)   #[1,1024,H/16,W/16]  
        ...
        x = self.conv5(x)   #[1,2048,H/16,W/16]
        #print(x.shape)
        # x = self.bn5(x)
        # x = self.relu(x)
        self.layers.append(x)

        return x,low_level_feat,mid_layer,third_layer,last_layer

    def get_layers(self):
        return self.layers

def xception65(pretrained=True, output_stride=16):
...
if __name__ == "__main__":
    import torch
    net = xception65(pretrained=True, output_stride=16)
    #model = AlignedXception(BatchNorm=nn.BatchNorm2d, pretrained=True, output_stride=16)
    input = torch.rand(1, 3, 512, 512)
    x,low_level_feat,mid_layer,third_layer,last_layers = net(input)
    #print(output.size())
